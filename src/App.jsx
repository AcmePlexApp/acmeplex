...
function App() {
	...
	const { setTheaters } = useTheaters();
	useEffect(() => {
		async function mapAndLogTheaters() {
			if (movies.length > 0) {
				const theaters = mapTheaters(movies);
				setTheaters(theaters);
				console.log("Mapped theaters:", theaters);
			}
		}
		mapAndLogTheaters();
	}, [movies, setTheaters]);
	...
	return (
		<Routes>
			...
			/>
			<Route
				path="/theater/:theaterId/showtime/:showtimeId/seats"
				element={
